...
def RunModel(seed, paramSU, paramINF) :
    ...
    classes.dS = paramSU
    classes.dI = paramINF
    fonctions.dS = paramSU
    fonctions.dI = paramINF
    ...
    DispersalS = classes.Event(name='Dispersal S',propensity='dS*self.S', Schange='-1', Ichange='0', order=1,EvolvingTrait=Evoltrait)
    DispersalI = classes.Event(name='Dispersal I',propensity='dI*self.I', Schange='0', Ichange='-1', order=1,EvolvingTrait=Evoltrait)
                    ...
                    if 'Dispersal' in event.name :
                        ...
                        if event.name == 'Extinction' :
                            ...
                            if abs(event.Ichange) > 0:  # If the event has an effect on 'evolving population'
                                print('EFFECTIFS Site', Site.effectifI, Site.effectifS)
                                print('CARACTERISTIQUES SITE',Site.traitvalues, Site.betaI, Site.Index)
                                Site.effectifS += trigger_persite[index] * event.Schange
                                Site.effectifI += trigger_persite[index] * event.Ichange
    ...
    datadensity.to_csv('Metapop_outputs_ESSfigure_0811' + '_' + str(dS) + '-' + str(dI) + '_' + str(seed) + '.csv')
    ...
    datatrait.to_csv('Traits_outputs_ESSfigure_0811' + '_' + str(dS) + '-' + str(dI) + '_' + str(seed) + '.csv')
    ...
    datadistrib.to_csv('Distribution_outputs_ESSfigure_0811' + '_' + str(dS) + '-' + str(dI) + '_' + str(seed) + '.csv')

################## MULTIPROCESSING PART ################

# Multiprocessing parameters
list_seeds = [1,2,3,4,5,6] # The list of seed you want to test
list_paramsS =[0.5] # The list of params values you want to test (has to be changed also at the beginning)
list_paramsI =[0.1]
#list_config =["Island"]
...
if __name__ == '__main__':
    ...
    for j in range(len(list_paramsS)) :
        for i in range(nbsims):
            ...
            #pool.apply_async(RunModel, args=(list_seeds[i],list_paramsS[j],list_paramsI[j])) #Launch multisim
            RunModel(list_seeds[i],list_paramsS[j],list_paramsI[j]) #Launch without multisim (restores errors messages)
